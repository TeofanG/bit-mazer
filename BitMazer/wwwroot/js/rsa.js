window.generateRSAkey = async function (mode, modulusLength, hashType) {
    try {
...
window.encryptRSA = async function (encPublicKey, plainData) {
    try {
        if (!key) {
...
window.decryptRSA = async function (encPrivateKey, encryptedData) {
    try {
...
window.importRSAprivateKey = async function (file) {
    try {
...
window.importRSApublicKey = async function () {
    try {
...
window.exportRSAkeysToFiles = async function (RSAkeyPair) {
    try {
...
window.getRSAkey = async function (isCustomKeyEnabled) {
    try {
        if (isCustomKeyEnabled) {
            ...
            const rsaKeyPair = await generateRSAkey("RSA-OAEP", 2048, "SHA-256");
